...
/* Star ratings: drag or hover across the row, release/click to set — half-star steps, 0 clears it */
const STAR_CELLS='<span class="star"><span class="s-bg">★</span><span class="s-fg">★</span></span>'.repeat(5);
function setFill(el,v){
 el.querySelectorAll('.star').forEach((cell,i)=>{
  const pct=Math.max(0,Math.min(100,(v-i)*100));
  cell.querySelector('.s-fg').style.clipPath=`inset(0 ${100-pct}% 0 0)`;
 });
}
function paintStars(el,v){el.dataset.value=v;setFill(el,v);}
function revealStars(el,v){
 el.classList.add('revealing');setFill(el,0);
 void el.offsetHeight;
 requestAnimationFrame(()=>{paintStars(el,v);setTimeout(()=>el.classList.remove('revealing'),550);});
}
function valueAt(el,clientX){
 const r=el.getBoundingClientRect(),x=clientX-r.left;
 return Math.round(Math.min(5,Math.max(0,x/r.width*5))*2)/2;
}
